...
SE3::SE3(Quaterniond &q, Matrix<double, 3, 1> &T, Matrix<double, 6, 1> &Lie):q_(q),T_(T),Lie_(Lie)
{

}

SE3::SE3(SE3& se3):q_(se3.q_),T_(se3.T_),Lie_(se3.Lie_)
{
    q_.normalize();
}
SE3&  SE3::operator=(SE3 &se3)
{
    q_=se3.q_;
    T_=se3.T_;
    Lie_=se3.Lie_;
...
SE3::SE3(Eigen::Matrix<double, 6, 1> &lie):Lie_(lie)
{
   // SE3 temp=exp(Lie_);
    q_=exp(Lie_).q_;
    T_=exp(Lie_).T_;
...
SE3::SE3(Eigen::Matrix3d& R, Eigen::Matrix<double, 3, 1>& T):q_(R.transpose()),T_(T)
{
    log();
    q_.normalize();
    std::cout<<"q: "<<q_.w()<<" "<<q_.x()<<" "<<q_.y()<<" "<<q_.z()<<std::endl;
}

Eigen::Quaternion<double > SE3::expAndTheta(Eigen::Vector3d const &omega, double *theta)
{
    using std::abs;
    using std::cos;
    using std::sin;
    ...
    *theta = sqrt(theta_sq);
    ...
    //q.normalize();
   // q.normalize();
    std::cout<<"exp:"<<q.w()<<" "<<q.x()<<" "<<q.y()<<" "<<q.z()<<std::endl;
    double qnorm=q.w()*q.w()+q.x()*q.x()+q.y()*q.y()+q.z()*q.z();
    q.w()/=qnorm;
    q.x()/=qnorm;
    q.y()/=qnorm;
    q.z()/=qnorm;
    std::cout<<"after exp:"<<q.w()<<" "<<q.x()<<" "<<q.y()<<" "<<q.z()<<std::endl;
    return q;
}

Matrix3d SE3::hat(Eigen::Vector3d &omega)
{
    Matrix3d M;
    M<<0,-omega.z(),omega.y(),
            omega.z(),0,-omega.x(),
...
SE3 SE3::exp(Eigen::Matrix<double ,6,1> & a)
{
    using std::cos;
    using std::sin;
...
void SE3::predictOutput(Eigen::VectorXd &input, Eigen::Matrix3d &K, Eigen::VectorXd &output)
{
   // std::cout<<"enter predictOutput.\n";
    Eigen::Matrix3d R=q_.inverse().toRotationMatrix();
   ...
   // std::cout<<"exit predictOutput.\n";
}

void SE3::computeJacobian(Eigen::VectorXd &input, Eigen::Matrix3d &K, Eigen::MatrixXd &jmat)
{
...
*/
        ...
        jmat(2*i,3)=fx*X*Y/(Z*Z);
        jmat(2*i,4)=-(fx+fx*X*X/(Z*Z));
        jmat(2*i,5)=fx*Y/Z;
        jmat(2*i,0)=-fx/Z;
        jmat(2*i,1)=0;
        jmat(2*i,2)=fx*X/(Z*Z);

        jmat(2*i+1,3)=fy+fy*Y*Y/(Z*Z);
        jmat(2*i+1,4)=-fy*X*Y/(Z*Z);
        jmat(2*i+1,5)=-fy*X/Z;
        jmat(2*i+1,0)=0;
        jmat(2*i+1,1)=-fy/Z;
        jmat(2*i+1,2)=fy*Y/(Z*Z);
...
void SE3::GN(Eigen::VectorXd &inputs, Eigen::VectorXd &measOutput, Eigen::Matrix3d &K)
{
...
TangentAndTheta SE3::logAndTheta()
{
    TangentAndTheta J;
    using std::abs;
    using std::atan;
...
void SE3::update(Eigen::Matrix<double, 6, 1> &dL)
{
    //SE3 tempSE=exp(dL);
    Matrix4d Td;
    Td.setIdentity();
    Td.block<3,3>(0,0)=exp(dL).q_.inverse().toRotationMatrix();
    Td.block<3,1>(0,3)=exp(dL).T_;

    std::cout<<"T:\n"<<Td<<std::endl;

    Matrix4d Tnow;
    Tnow.setIdentity();
    Tnow.block<3,3>(0,0)=q_.inverse().toRotationMatrix();
